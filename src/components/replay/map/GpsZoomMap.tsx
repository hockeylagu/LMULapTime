...
import { GpsZoomControls } from './GpsZoomControls.js';
    ...
    >
      <div className="absolute top-2 left-2.5 right-2.5 z-20 flex items-center justify-between pointer-events-none">
        ...
        <GpsZoomControls
          ...
          onZoomIn={() => setZoomRadius((r) => Math.max(15, Math.round(r * 0.8)))}
          onZoomOut={() => setZoomRadius((r) => Math.min(300, Math.round(r * 1.25)))}
          onSelectRadius={(r) => {
            setZoomRadius(r);
            setPanOffset({ x: 0, y: 0 });
          }}
          onRecenter={() => setPanOffset({ x: 0, y: 0 })}
          ...
          <g transform={panOffset.x !== 0 || panOffset.y !== 0 ? `translate(${panOffset.x.toFixed(1)}, ${panOffset.y.toFixed(1)})` : undefined}>
            ...
            <g data-track-line="baseline" pointerEvents="none">
              {baselineSegments.map((seg, i) => (
                <path
                  key={`baseline-${i}`} d={seg.pathD} stroke={seg.color} strokeWidth="5"
                  strokeDasharray="10 7" strokeLinecap="round" strokeLinejoin="round" fill="none" opacity={baselineOpacity}
                />
              ))}
            </g>
            {baselineGhostPos && (
