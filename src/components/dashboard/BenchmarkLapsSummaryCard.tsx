      ...
      <div className={`space-y-1.5 flex-1 ${showMoreBenchmarks ? 'max-h-60 overflow-y-auto custom-scrollbar pr-0.5' : ''}`}>
        {visibleRefLaps.length > 0 ? (
          visibleRefLaps.map((item, idx) => {
            const paceStyle = getPaceCategoryStyle(item.category);
            return (
              <div
              ...
              >
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <span className={`${paceStyle.textClass} font-mono font-bold text-[11px]`}>
                    {item.percentage.toFixed(1)}%
                  </span>
                  <span className="text-[10px]">{paceStyle.emoji}</span>
                </div>
              </div>
            );
          })
